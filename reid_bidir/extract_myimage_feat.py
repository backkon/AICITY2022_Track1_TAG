...
def load_all_data(reid_img_path):
    """Load all mode data."""

    image_list = []
    for cam in os.listdir(reid_img_path):
        if cam not in {'c041', 'c042', 'c043', 'c044', 'c045', 'c046'}:
            continue
        image_dir = os.path.join(reid_img_path, cam, 'dets')
...
def save_feature(output_path, reid_img_path, pool_output):
    """Save feature."""
    ...
    for cam in os.listdir(reid_img_path):
        if cam not in {'c041', 'c042', 'c043', 'c044', 'c045', 'c046'}:
            continue
        dets_pkl_file = os.path.join(reid_img_path, cam, f'{cam}_dets.pkl')
        det_dic = pickle.load(open(dets_pkl_file, 'rb'))
        reid_feat_dic[cam] = det_dic.copy()

    for sample_dic in pool_output:
        for image_path, feat in sample_dic.items():
            cam = image_path.split('/')[-3]
            image_name = image_path.split('/')[-1].split('.')[0].replace("_flip","")
            flip = True if image_path.split('/')[-1].split('.')[0].split('_')[-1]=="flip" else False
            if flip:
                reid_feat_dic[cam][image_name]['feat_flip'] = feat
            else:
                reid_feat_dic[cam][image_name]['feat'] = feat
    for cam, feat_dic in reid_feat_dic.items():
