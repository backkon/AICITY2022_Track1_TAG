...
def eval_seq(seq, pp='', split='train', mcmt_cfg=None):
    ...
    # post associate
    results = associate(results, 0.3, 10, seq)   # 0.3
    results = associate(results, 0.2, 10, seq)   # 0.2

    # remove len 1 track and interpolate, help on reducing FNs.
    # results = interpolate_traj(results, drop_len=10)

    # track nms can help reduce FPs.
    results = track_nms(results, 0.65)  # 参数调大: 0.65
