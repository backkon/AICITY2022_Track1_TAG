...
def IsPointInPolygon(pt, poly):
    """
        点是否在多边形区域内(在边上不算）https://www.cnblogs.com/dwdxdy/p/3230647.html
        @param poly: 多边形顶点数组 [[x0, y0], [x1, y1], ...]
        @param pt: 目标点 [x, y]
        @return True/False
    """
    c = False
    for i in range(len(poly)):
        j = (i + 1) % len(poly)
        # if (pt[0] - poly[i][0]) * (poly[j][1] - poly[i][1]) - (poly[j][0] - poly[i][0]) * (pt[1] - poly[i][1]) == 0:
        #     return False
        if((poly[i][1] > pt[1]) != (poly[j][1] > pt[1])) and (pt[0] < (poly[j][0]-poly[i][0]) * (pt[1]-poly[i][1]) / (poly[j][1] - poly[i][1]) + poly[i][0]):
            c = not c
    return c
...
def run(sequence_dir, detection_file, output_file, min_confidence,
    ...
    tracker = FairTracker(min_confidence, seq_info["frame_rate"])
    results = []
    ...
    def frame_callback(vis, frame_idx):
        ...
        #     cx = (det[2]+det[0])*0.5
        #     cy = (det[3]+det[1])*0.5
        #     # if (det[2]-det[0])*(det[3]-det[1]) > args.min_box_area:
        #     if IsPointInPolygon((cx, cy), polys):
